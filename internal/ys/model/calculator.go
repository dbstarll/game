...
func (c *Character) Calculate(enemy *Enemy) *Calculator {
	...
	// 源数据
	values["人物攻击力"] = c.base.Get(point.Atk).value
	values["武器攻击力"] = c.weapon.base.Get(point.Atk).value
	for _, p := range point.Points {
		if v := finalAttributes.Get(p); !v.IsZero() {
			if p.IsPercentage() {
				values[p.String()] = v.value / 100
			} else {
				values[p.String()] = v.value
			}
		}
	}
	values["人物等级"] = float64(c.level)
	values["怪物等级"] = float64(enemy.level)

	// 基础区
	values["基础攻击力"] = values["人物攻击力"] + values["武器攻击力"]
	values["额外攻击力"] = values["基础攻击力"]*values["攻击力%"] + values["攻击力"]
